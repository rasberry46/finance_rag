...
def relevancy(ans, q):
    return score("Score 0.0-1.0 how well ANSWER addresses QUESTION. Reply ONLY the number.",
                 "QUESTION: " + q + "\n\nANSWER: " + ans + "\n\nScore:")
...
for q, gt in GOLDEN:
    ...
    print("\nQ: " + q)
    print("   answer: " + ans[:65].strip())
    print("   faithfulness=%.2f relevancy=%.2f ctx_precision=%.2f ctx_recall=%.2f" % (f, r, p, rc))
n = len(GOLDEN)
print("\n" + "-" * 72)
print("AVERAGES: faithfulness=%.2f relevancy=%.2f ctx_precision=%.2f ctx_recall=%.2f" % (
    sums["f"]/n, sums["r"]/n, sums["p"]/n, sums["rc"]/n))
print("=" * 72)
